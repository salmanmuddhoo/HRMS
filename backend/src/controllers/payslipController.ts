...
export const generatePayslip = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [companyNameCfg, companyAddressCfg, companyPhoneCfg, companyEmailCfg] = await Promise.all([
    ...
    const payslipData = {
      ...
      payroll: {
        ...
        leaveDays: Number(payroll.leaveDays),
        localLeaveDays,
        sickLeaveDays,
        ...
        compensations: (payroll.compensations || []).map(c => ({
          label: c.label,
